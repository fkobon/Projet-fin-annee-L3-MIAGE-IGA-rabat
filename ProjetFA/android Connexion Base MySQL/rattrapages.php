...
$result = mysqli_query($db->connection(),"SELECT NOM_PROF,DATE_RAT,SALLE FROM absences a join professeurs p on a.CIN_PROF = p.CIN_PROF
										join abs_concern ac on a.NUM_ABS = ac.NUM_ABS join salles s on a.NUM_SAL_RAT = s.NUM_SALLE where ac.NUM_GR = 
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_array($result)) {
		...
		$ligne["DATE_RAT"] = $row["DATE_RAT"];
		$ligne["SALLE"] = $row["SALLE"];
